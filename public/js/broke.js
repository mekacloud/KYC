...
$(document).on('ready', function() {
	...
	$('#getinfobutton').click(function(){
		...
		//if (e.which == 13) {
			ws.send(JSON.stringify({type: 'getcus', cusid:$('input[name="cusid"]').val(), v: 1}));
		//}
	})
	...
	function showHomePanel(){
		...
		setTimeout(function(){
			...
			ws.send(JSON.stringify({type: 'getcustomerofbroke', v: 1}));						//need to wait a bit
...
function connect_to_server(){
	...
	function onOpen(evt){
		...
		ws.send(JSON.stringify({type: 'getcustomerofbroke', v:1, brokeno:bag.setup.BROKER1NO}));
		ws.send(JSON.stringify({type: 'chainstats', v:1}));
	}
