...
module.exports.process_msg = function (ws, data) {
	...
	if (data.v === 'br') {
		if (data.type == 'getcus') {
			...
			chaincode.query.readcustomer([data.cusid], cb_got_cus);
		}
			...
			try {
				...
				if (broker.allowcustomer) {
					...
					async.eachLimit(keys, concurrency, function (key, cb) {
						...
						chaincode.query.readcustomergid([broker.allowcustomer[key]], function (e, customerinfo) {
							if (e != null) console.log('[ws error] did not get customer:', e);
							else {
								var customer = JSON.parse(customerinfo);
								console.log('! +++', customer.customerid);
								chaincode.query.readcustomer([customer.customerid], function (e, customer) {
				...
				if (broker.pendingcustomer) {
					...
					async.eachLimit(keys, concurrency, function (key, cb) {
						...
						chaincode.query.readcustomergid([broker.pendingcustomer[key]], function (e, customerinfo) {
							if (e != null) console.log('[ws error] did not get customer:', e);
							else {
								var customer = JSON.parse(customerinfo);
								console.log('! +++', customer.customerid);
